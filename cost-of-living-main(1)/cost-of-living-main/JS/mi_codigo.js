...
function principal(){
  /***    ###########################################
    ...
    let lista_nombre = ciudades.map(x => x.nombre);
    ...
    lista_nombre.forEach(c => console.log(c));
    ...
    */
    ...
    tomates_medio_Alemania= ciudades.filter(ciudad => ciudad.nombre.includes("Germany")).map(c=> parseFloat(precio_tomate));
    tomates_medio_Alemania= tomates_medio_Alemania.reduce((total,x,index,array)) => {
        total += x;
